...
router.get('/:id', (req, res) => {
    let sqlText = `SELECT * FROM "cross_stitch_pattern" WHERE "id" = $1`;
    pool.query(sqlText, [req.params.id]).then( response =>{
        res.send(response.rows);
    }).catch( error => {
        console.log ('error in getting cross stitch pattern with id:', req.params.id, error);
        res.sendStatus(500);
    });
});

router.get('/', (req, res) => {
    let sqlText = `SELECT * from "cross_stitch_pattern" ORDER BY "id";`;
    pool.query(sqlText).then( response => {
        console.log('all the cross stitch patterns', response.rows);
        res.send(response.rows);
    }).catch( error => {
        console.log('error in getting the cross stitch pattern list', error);
        res.sendStatus(500);
    });
})
...
router.post('/', (req, res) => {
    ...
    let sqlText = `INSERT INTO "cross_stitch_pattern" ("pattern") VALUES($1)`;
    pool.query(sqlText, [req.body.pattern]).then( () =>{
        res.sendStatus(200);
    }).catch( error => {
        console.log('error in adding that cross stitch pattern to the database', error);
        res.sendStatus(500);
    });
});
